      ...
      >
        <div className="flex items-center justify-between">
          <button 
            ...
            className="w-14 h-14 bg-white rounded-2xl shadow-sm border border-stone-100 flex items-center justify-center active:scale-90 transition-all"
          >
          ...
          <div className="flex gap-2">
            {[0, 1, 2, 3, 4].map((s) => (
              <div 
                key={s} 
                className={`h-2 rounded-full transition-all duration-500 ${s === step ? 'w-10 bg-amber-500' : 'w-2 bg-stone-200'}`} 
              />
            ))}
          </div>
          <div className="w-14" />
            ...
            >
              {step === 0 && (
                <div className="space-y-8">
                  <div className="space-y-3">
                    <h2 className="text-3xl font-light serif text-stone-950">Language</h2>
                  ...
                  <div className="grid grid-cols-2 gap-3 max-h-[300px] overflow-y-auto pr-2 no-scrollbar">
                    {LANGUAGES.map(lang => (
                      <button
                        ...
                        className={`p-6 rounded-[2rem] border-2 transition-all text-sm font-bold flex items-center justify-between ${selectedLang === lang.name ? 'border-amber-500 bg-amber-50 text-amber-600' : 'border-stone-50 bg-stone-50 text-stone-400'}`}
                      >
                        {lang.label}
              ...
              {step === 1 && (
                <div className="space-y-10">
                  <div className="space-y-3">
                    <h2 className="text-3xl font-light serif text-stone-950">Identity</h2>
              ...
              {step === 2 && (
                <div className="space-y-10">
                  <div className="space-y-3">
                    <h2 className="text-3xl font-light serif text-stone-950">Mobile</h2>
              ...
              {step === 3 && (
                <div className="space-y-10">
                  <div className="space-y-3">
                    <h2 className="text-3xl font-light serif text-stone-950">Farm</h2>
              ...
              {step === 4 && (
                <div className="space-y-8">
                  <div className="space-y-3">
                    <h2 className="text-3xl font-light serif text-stone-950">Location</h2>
                  ...
                  <div className="space-y-4">
                    <div className="relative">
                      <select 
                        ...
                        className="w-full bg-stone-50 border-2 border-stone-100 p-6 pl-16 rounded-[2rem] outline-none focus:border-amber-500 transition-all font-bold text-stone-900 appearance-none"
                      >
                        <option value="">Select State</option>
                        {STATES.map(s => <option key={s} value={s}>{s}</option>)}
                      </select>
                      <MapPin className="absolute left-6 top-1/2 -translate-y-1/2 text-stone-400 w-6 h-6" />
                    </div>
                    <InputField icon={<MapPin className="w-6 h-6" />} placeholder="District" value={district} onChange={setDistrict} />
                    <InputField icon={<Landmark className="w-6 h-6" />} placeholder="Mandal / Tehsil" value={mandal} onChange={setMandal} />
                    <InputField icon={<HomeIcon className="w-6 h-6" />} placeholder="Revenue Village" value={revenue} onChange={setRevenue} />
                  </div>
                </div>
              )}
